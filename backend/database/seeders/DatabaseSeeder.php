...
class DatabaseSeeder extends Seeder
{
    public function run(DefaultCategories $defaults, TransactionService $transactions): void
    {
        $user = User::query()->firstOrCreate(
            ...
            ['name' => 'Demo User', 'password' => 'password1', 'locale' => 'ar', 'currency' => 'SAR'],
        );
        ...
        $bank = $user->accounts()->create(['name' => 'Al Rajhi', 'type' => 'bank', 'currency' => 'SAR', 'opening_balance' => 1_250_000, 'color' => '#0F7A68', 'icon' => 'account_balance']);
        $bank->forceFill(['balance' => $bank->opening_balance])->save();
        $cash = $user->accounts()->create(['name' => 'Cash', 'type' => 'cash', 'currency' => 'SAR', 'opening_balance' => 50_000, 'color' => '#F5B83D', 'icon' => 'payments']);
        ...
        $transactions->create($user, [
            ...
            'amount' => 1_500_000, 'occurred_at' => now()->startOfMonth()->addDays(1), 'payee' => 'Employer',
        ]);
        ...
        foreach (range(1, 40) as $i) {
            $transactions->create($user, [
                ...
                'category_id' => $categories[fake()->randomElement(['food', 'groceries', 'transport', 'bills', 'shopping', 'entertainment'])],
